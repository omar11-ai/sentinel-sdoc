...
/**
 * Apple Liquid Glass ThemeSwitcher — adapted for SENTINEL.
 * Origin: 21st.dev community component (Den Dionigi's "Liquid glass" pen).
 * The glass refraction is a real SVG displacement map (feImage + feDisplacementMap)
 * applied through backdrop-filter — the SVG defs are shared app-wide via
 * <LiquidGlassDefs /> so buttons and menus can refract through the same glass.
 */
...
/**
 * Mount once (e.g. in the app root). Hosts the SVG displacement filters that
 * power every liquid-glass surface: #lg-switcher (strong refraction) and
 * #lg-toggler (subtle refraction). Hidden from layout, never display:none.
 */
export function LiquidGlassDefs() {
  return (
    ...
    >
      <svg>
        <filter id="lg-switcher" primitiveUnits="objectBoundingBox">
          <feImage
            result="map"
            width="100%"
            height="100%"
        ...
        <filter id="lg-toggler" primitiveUnits="objectBoundingBox">
          <feImage
            result="map"
            width="100%"
            height="100%"
...
export function ThemeSwitcher({
  ...
  return (
    <fieldset className="switcher" data-previous={previousValue}>
